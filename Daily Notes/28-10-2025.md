...
<< [[27-10-2025|Yesterday]] | Tuesday, October 28, 2025 | [[29-10-2025|Tomorrow]] >>
...
```mermaid
flowchart 
    subgraph Input["🖥️ Terminal Input"]
        A[User enters values\n e.g. temperature=23.5]
    end
    subgraph Args["💡 Program Arguments"]
        B["argv[] (char*)\nText strings"]
    end
    subgraph Struct["📦 struct Telemetry"]
        C["temperature: float (4B)\n battery: int (4B)\n gps_lock_state: int (4B)\n→ Total ≈ 12 bytes"]
    end
    subgraph Union["🔀 union FloatBytes"]
        D["float flt (4B)\nbyte[4] bytes"]
    end
