...
>We say $f(x)\text{ is }\Omega(g(x))$ if there are positive constant $C$ and $k$ such that: $$|f(x)|\ge C\cdot |g(x)|$$whenever $x > k$. Note that this implies that $f(x)\text{ is }\Omega(g(x))$ if and only if $g(x)\text{ is }O(f(x))$.
